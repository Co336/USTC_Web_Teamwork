    ...
    for lst in hash_list[hash_function(word)]:
        if lst[0] == word:
            with shelve.open(file_index_list) as db:
                id_list = db.get(str(lst[1]), None)  # 通过索引读取数据
                return [decompress_ids(id_list[0]),decompress_ids(id_list[1]),id_list[2]]
    return [[],[]]
...
def Book_bool_query():
    # 载入hash表
    hashlist = hash_load('jieba\data\\book_hash_list.pkl')
    # 获取倒排索引表文件
    file_inverted_index = 'jieba\data\compressed_book_index'
    # 获取所有文档id
...
def Movie_bool_query():
    # 载入hash表
    hashlist = hash_load('jieba\data\\movie_hash_list.pkl')
    # 获取倒排索引表文件
    file_inverted_index = 'jieba\data\compressed_movie_index'
    # 获取所有文档id
