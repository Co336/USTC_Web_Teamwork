...
def Book_bool_query():
    # 获取倒排索引表
    inverted_index = csv_to_dict(file_book_index_table_csv)
    # 获取所有文档id
    all_docs = get_all_name("part-3 bool-query\\bookname.txt")
    # 创建一个查询解析器并执行查询
    parser = BooleanQueryParser(inverted_index,all_docs)
...
def Movie_bool_query():
    # 获取倒排索引表
    inverted_index = csv_to_dict(file_movie_index_table_csv)
    # 获取所有文档id
